...
首次采样时 CPU、GPU、网络和硬盘速率可能为 0；第二次请求后会根据两次累计计数的差值计算。小电视每 1 秒请求一次 `/status`。

`GET /net` 使用同一 Bearer Token，只读取所选网卡的累计计数，适合高频轮询。它不采集 CPU、GPU、内存、温度、磁盘或容器：

```json
{"sample_time":123456.25,"iface":"physical:enp2s0,enp3s0","rx_bytes":1690000000000,"tx_bytes":394000000000}
```
...
`disk_io.read_speed` 和 `disk_io.write_speed` 是全部物理块设备在相邻两次 `/status` 请求间的合计字节速率。服务扫描 `/sys/class/block`，忽略分区、loop、device-mapper 和 md 等逻辑层，避免同时统计逻辑卷与底层硬盘。RAID1 等阵列会按每块物理盘的实际 I/O 合计，因此写入值可能高于应用层写入量。计数器不可读时 `valid` 为 `false`，速率返回 0。
