...
function renderOverview() {
  ...
  text('storage-detail',d.storage.valid?`${d.storage.percent}% 已用 · ${d.storage.filesystems} 个文件系统（已去重）`:'未读取到已挂载的数据卷');
  ...
  const rows=[['CPU / 内存',d.sources.cpu+' / '+d.sources.memory],['GPU',d.sources.gpu === 'unavailable'?'未支持当前 GPU 采集方式':d.sources.gpu],['数据卷',d.sources.storage_paths.join('、')],['UPS',`${d.ups.source} · ${d.ups.valid?'有效':'不可用'}${d.ups.reason?' · '+d.ups.reason:''}${d.ups.age_seconds!=null?' · '+d.ups.age_seconds+'秒前':''}`],...d.temp.map(sensor=>[sensor.type,`${sensor.temp} °C`])];
...
async function loadInterfaces(reset) {
  ...
  if(reset||!state.saved){state.saved=structuredClone(data.settings);state.draft=structuredClone(data.settings);state.previewValid=false;}
...
document.querySelectorAll('[data-mode]').forEach(button=>button.addEventListener('click',()=>{state.draft.mode=button.dataset.mode;if(state.draft.mode==='recommended')state.draft.members=state.interfaces.filter(row=>state.network.recommended.includes(row.id)).map(row=>({id:row.id,name:row.name}));if(state.draft.mode==='single')state.draft.members=state.draft.members.slice(0,1);preview();}));
$('interfaces').addEventListener('change',event=>{const id=event.target.dataset.select;if(!id)return;const row=state.interfaces.find(row=>row.id===id);if(state.draft.mode==='recommended')state.draft.mode='sum';if(event.target.checked){if(state.draft.mode==='single')state.draft.members=[];state.draft.members.push({id,name:row.name});}else state.draft.members=state.draft.members.filter(member=>member.id!==id);preview();});
