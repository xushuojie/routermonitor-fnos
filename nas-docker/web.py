...
class WebApp:
    ...
    def valid_password(self, password):
        if not isinstance(password, str) or len(password) > 128:
            return False
        value = hashlib.pbkdf2_hmac('sha256', password.encode(), self.password['salt'].encode(), 300000).hex()
        return hmac.compare_digest(value, self.password['hash'])
    ...
    def same_origin(self, handler):
        ...
        return origin.scheme in ('http', 'https') and origin.netloc == handler.headers.get('Host') and not origin.path and not origin.username
    ...
    def handle(self, handler):
        ...
        try:
            ...
            if handler.command != 'GET':
                ...
                if path != '/api/login' and (session is None or not hmac.compare_digest(handler.headers.get('X-CSRF-Token', ''), session['csrf'])):
            ...
            if path == '/api/login' and handler.command == 'POST':
                ...
                with self.lock:
                    ...
                    if len(failures) >= 8 or len(self.failures) > 256:
                ...
                self.respond(handler, 200, {'authenticated': True, 'csrf': session['csrf']},
                             'nas_admin=' + sid + '; HttpOnly; SameSite=Strict; Path=/; Max-Age=28800')
                return True
            ...
            if handler.command == 'GET':
                ...
                self.respond(handler, 200, {'ok': True}, 'nas_admin=; HttpOnly; SameSite=Strict; Path=/; Max-Age=0')
                return True
        ...
        except (SelectionError, ValueError, TypeError, json.JSONDecodeError) as error:
            handler.close_connection = True
            self.respond(handler, 409 if '其他页面' in str(error) else 400, {'error': str(error)})
