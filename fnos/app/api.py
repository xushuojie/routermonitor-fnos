...
import hardware
from hardware_settings import intervals, load_settings
...
def cpu_sample():
    ...
    try:
        ...
        return values if len(values) >= 4 and min(values) >= 0 else None
    except ValueError:
        return None
...
class NetworkMonitor:
    ...
    def sample(self):
        ...
        with self.lock:
            ...
            if not same or not 0 < elapsed <= max(1, intervals()["network"] * 3):
                self.epoch = str(time.time_ns())
                self.samples.clear()
    ...
    def latest(self, include_counters=False):
        with self.lock:
            if self.current is None or time.monotonic() - self.current["sample_time"] > max(1, intervals()["network"] * 3):
                return None
    ...
    def response(self, since=0, epoch="", limit=4):
        with self.lock:
            ...
            if age > max(1, intervals()["network"] * 3):
                return None
    ...
    def run(self):
        if self.stop.wait(intervals()["network"]):
            return
        while not self.stop.is_set():
            ...
            self.stop.wait(max(.01, intervals()["network"] - (time.monotonic() - started)))
...
def storage_status():
    # The dedicated collector owns selection, identity checks and filesystem de-duplication.
    from storage_discovery import read_snapshot
    result = read_snapshot(os.environ.get("NAS_STATUS_STORAGE_SNAPSHOT", "/discovery/storage.json"))
    # Keep UI selection metadata on /api/hardware, outside the existing device payload.
    fields = ('path', 'filesystem', 'source', 'device', 'mount_root', 'valid', 'total', 'used', 'included', 'reason')
    result['volumes'] = [{key: row[key] for key in fields if key in row} for row in result.get('volumes', [])]
    if result.get('mode') == 'selected':
        result['mode'] = 'manual'
    return result


def gpu_sample():
    return hardware.gpu_sample()


def ups_status():
    return hardware.ups_status()
...
class UpsMonitor:
    ...
    def sample(self):
        ...
        signature = tuple(settings.get(key) for key in ("ups_mode", "ups_socket", "ups_host", "ups_port", "ups_name"))
        with self.lock:
    ...
    def run(self):
        while not self.stop.is_set():
            started = time.monotonic()
            self.sample()
            self.stop.wait(max(.01, intervals()["ups"] - (time.monotonic() - started)))
...
class Metrics:
    ...
    def snapshot(self, force=False):
        now = time.monotonic()
        settings = load_settings()
        cadence = intervals(settings)
        signature = tuple(settings.get(key) for key in ("cpu_sensor", "disk_sensor", "gpu", "profile"))
        if self.settings_signature != signature:
        ...
        if current_cpu and self.previous_cpu:
            ...
            if total > 0 and len(current_cpu) == len(old) and all(new >= before for new, before in zip(current_cpu[:8], old[:8])):
                cpu_valid = True
        ...
        try:
            ...
            gpu_backend, gpu_value, gpu_is_cumulative = "unavailable", 0.0, False
        gpu_id = hardware.LAST_GPU_ID
        if gpu_id != self.previous_gpu_id:
            self.previous_gpu = None
            self.previous_gpu_id = gpu_id
        gpu_percent = 0.0
        ...
        if self.cached_temperatures is None or now - self.temperatures_at >= cadence["sensors"]:
            try:
                self.cached_temperatures = temperatures()
            except Exception:
                self.cached_temperatures = []
            self.temperatures_at = now
        if self.cached_storage is None or now - self.storage_at >= cadence["storage"]:
            try:
                self.cached_storage = storage_status()
            except Exception:
                self.cached_storage = {"total": None, "used": None, "percent": None, "valid": False,
                                       "filesystems": 0, "mode": "auto", "reason": "存储采集不可用"}
            self.storage_at = now
    ...
    def read_snapshot(self):
        ...
        if storage.get('mode') == 'auto' and storage.get('sampled_at') is not None:
            ...
            if not 0 <= age <= max(90, intervals()["storage"] * 3):
    ...
    def run(self):
        if self.stop.wait(intervals()["status"]):
            return
        while not self.stop.is_set():
            started = time.monotonic()
            try:
            ...
            self.stop.wait(max(.01, intervals()["status"] - (time.monotonic() - started)))
...
def resolve_token():
    ...
    custom = custom_path(os.path.dirname(os.environ.get('NAS_STATUS_HISTORY_DB', DEFAULT_HISTORY_DB)))
    if custom.exists():
        return validate(custom.read_text(encoding='ascii').strip())
...
def handler_factory(metrics, token, network=None, web=None):
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            current_token = token() if callable(token) else token
            if not current_token or not hmac.compare_digest(supplied.encode('utf-8'), ("Bearer " + current_token).encode('utf-8')):
                self.close_connection = True
        ...
        def log_message(self, message, *args):
            ...
            if method == "GET" and getattr(self, 'path', '').split("?", 1)[0] in ("/net", "/status", "/api/overview", "/api/network/stream", "/api/network/interfaces", "/api/capabilities", "/api/session") and len(args) > 1 and str(args[1]) == "200":
                return
            ...
            print(json.dumps({'peer': self.client_address[0], 'method': method, 'status': status}), flush=True)
...
def main():
    ...
    server = LimitedHTTPServer((args.bind, args.port), handler_factory(metrics, web.device_tokens.current, network, web))
    metrics.ups.start()
    history.start()
    metrics.start()
